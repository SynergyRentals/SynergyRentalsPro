...
export default function PropertyCalendar({ events, isLoading }: PropertyCalendarProps) {
  ...
  const processedEvents = events.map(event => {
    try {
      ...
      const explicitCheckoutDate = event.checkout ? 
        (event.checkout instanceof Date ? event.checkout : parseISO(event.checkout as string)) :
        null;
      
      // Use our shared utility functions to ensure consistent handling with the server
      // Normalize dates to UTC midnight
      const normalizedStart = normalizeToUTCMidnight(startDate);
      const normalizedEnd = normalizeToUTCMidnight(endDate);
      
      // Calculate checkout date - use explicit date if provided, otherwise calculate using shared utility
      const normalizedCheckout = explicitCheckoutDate 
        ? normalizeToUTCMidnight(explicitCheckoutDate)
        : getCheckoutDate(normalizedEnd);
      
      // Debug date transformation in development mode
      if (process.env.NODE_ENV === 'development') {
        console.debug(`Event ${event.uid} date processing:
          ...
          Checkout: ${normalizedCheckout.toISOString()} (${explicitCheckoutDate ? 'explicit' : 'calculated'})
        `);
      }
      ...
      // Use shared fallback date utility to ensure consistency with server
      const fallbackDates = createFallbackDates();
      
      return {
        ...event,
        start: fallbackDates.start,
        end: fallbackDates.end,
        checkout: fallbackDates.checkout
      };
    }
  });
  ...
  const getEventsForDay = (day: Date) => {
    return processedEvents.filter(event => {
      ...
      const checkoutDate = event.checkout as Date;
      
      // Normalize the day using our shared utility function for consistent comparison
      const normalizedDay = normalizeToUTCMidnight(day);
